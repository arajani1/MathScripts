...
def holes(x_range_min,interval,y,fc):
    ...
    for val in nans:
        ...
        if val == True:
            nan.append(index)
            nany.append(eval(fc.replace("x",str(index+.001))))
    return nan,nany
...
if window_selection == 3:
    x_range_min = float(raw_input("x-minimum:"))
    x_range_max = float(raw_input("x-maximum:"))
    y_range_min = float(raw_input("y-minimum:"))
    y_range_max = float(raw_input("y-maximum:"))
    interval = .001
...
user_choice = str(raw_input("Type 'int' for definite integral algorithim and second ftc \n press enter to skip \n"))

#does integration and second ftc if user types ‘int’
if user_choice == 'int':
    lowerlimit = float(raw_input("input lower limit:"))
    upperlimit = float(raw_input("input upper limit:"))
    y = eval(fc)
